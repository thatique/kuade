...
function usage {
    ...
    >&2 echo "  KEYSPACE           - keyspace (default: kuade_v1_{datacenter})"
...
template=$1
if [[ "$template" == "" ]]; then
    template=$(ls schema/$(dirname $0)/*cql.tmpl | sort | tail -1)
fi
...
keyspace=${KEYSPACE:-"kuade_v1_${datacenter}"}
...
cat $template | sed \
    ...
    -e "s/\${session_ttl}/${session_ttl}/g"         | cat -s
